# Dívida de licença — obras em regime integral

<!-- Gerado por `npm run validate:content -- --write`. Não editar à mão: sem
     a flag, qualquer divergência é reprovada como DIVIDA_DESATUALIZADA. -->

O **regime integral** (§1.1 do `docs/SOURCE-CORPUS.md`) desliga o teto de citação
e o teto de rotação de livro-base para uma obra protegida — a obra continua
protegida, e é por isso que esta lista existe. É a lista de troca: tudo que sai
dessas obras e precisa virar fonte pública no dia em que o curso deixar de ser
gratuito.

## 100 Endgames You Must Know — Jesús de la Villa

- **slug:** `de-la-villa-100`
- **desde:** 2026-09-08
- **prazo:** 2027-09-08
- **motivo:** Regime aberto em 2026-09-08, quando este livro era a base do módulo de finais, e **mantido em 2026-09-12 com o papel trocado**: o módulo inteiro passou a seguir o Silman, e esta obra fica registrada por um acervo legado — a aula N1-KPK, escrita antes da decisão e não reancorada, e as duas posições já transcritas. O regime continua porque é ele que mantém esse acervo inventariado: o que a protege não é o teto de citação, e sim esta declaração, com prazo e inventário. O preparatório é gratuito e não será comercializado por ora; quando for, o conteúdo listado em content/divida-de-licenca.md será trocado por fonte pública.

### Aulas com esta obra como livro-base (1)

- `N1-KPK` — classe D, "Rei e peão contra rei: o rei na frente do peão"

### Posições que citam esta obra (2)

- `pos-n1-kpk-dlv-1-3` — de la Villa, 100 Endgames You Must Know, New In Chess, Alkmaar, 2008, p. impressa 29 (PDF p. 29, offset 0), capítulo 1 "Basic endings", Posição 1.3, com a linha impressa 1.Kc7! Ke6 2.b4 Kd5 3.b5 1-0
- `pos-n1-square-dlv-1-1` — de la Villa, 100 Endgames You Must Know, p. impressa 27 (PDF p. 27, offset 0), cap. 1 'Basic endings', ENDING 1 'The rule of the square', Position 1.1

## Silman's Complete Endgame Course — Jeremy Silman

- **slug:** `silman-endgame-course`
- **desde:** 2026-09-08
- **prazo:** 2027-09-08
- **motivo:** Decisão do Doug em 2026-09-08: todas as aulas de finais passam a seguir o Silman, texto e posições. É o único manual do acervo com progressão declarada por classe de força, que é exatamente a espinha que a trilha precisa, e o preparatório é gratuito. Quando o curso for comercializado, o conteúdo listado em content/divida-de-licenca.md será trocado por fonte pública.

### Aulas com esta obra como livro-base (2)

- `N0-LADDER` — classe E, "Mate da escada: duas torres, e dama e torre"
- `N0-MATING-MATERIAL` — classe E, "O que dá mate e o que não dá"

### Posições que citam esta obra (2)

- `pos-n0-ladder-silman-yk7` — Silman, Silman's Complete Endgame Course, Siles Press, Los Angeles, 2007, Parte Um 'Endgames for Beginners (Unrated-999)', seção 'King and Two Rooks vs. Lone King' — página impressa não conferida. A seção é o capítulo 7 do estudo que retranscreve o livro, e o nome acima é o do capítulo, não o da página impressa.
- `pos-n0-mating-material-silman-38` — Silman, Silman's Complete Endgame Course, Siles Press, Los Angeles, 2007, p. impressa 34 (PDF p. 48, offset 14), Parte Dois 'Endgames for Class E (1000-1199)', seção 'What Can (or Cannot) Mate vs. Lone King' (impressas 33-34), Diagram 38 — o único diagrama da seção

