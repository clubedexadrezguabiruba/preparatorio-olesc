...
import { RESPOSTA_MS } from "./tatica/tempos.ts";
 ...
 * dá um som que o catálogo não sabe nomear.
 *
 * **O teto de duração é o que este gate tem de mais duro aqui.** Os sons vieram
 * do laboratório de finais, onde o intervalo até a resposta do adversário é de
 * 620 ms; na série de tática ele é de 480 ms (`RESPOSTA_MS`). Um som de lance
 * mais longo que esse intervalo empilha dois lances num barulho só. Os três
 * tetos declarados passam — e é por isso que o teste existe: abaixar o
...
test("lance, captura e xeque cabem no intervalo da resposta do adversário", () => {
  ...
  for (const nome of ["lance", "captura", "xeque"] as const) {
    ...
    assert.ok(
      effect.maxDurationMs < RESPOSTA_MS,
      `${nome}: teto de ${effect.maxDurationMs} ms passa dos ${RESPOSTA_MS} ms da resposta`,
    );
  }
});
