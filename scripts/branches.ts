...
export function equivalentCandidates(node: TreeNode, winning: string[]): string[] {
  ...
  // O que o autor já declarou fica de fora — erro nomeado e lance declarado
  // válido (B8.2). No caso do `authorAlternatives` isso não é cortesia: na
  // etapa 4 o gerador transformaria o lance num `expect` de verdade, e
  // `judgeMove` consulta `expects` **antes** — o texto que o autor escreveu
  // para aquele lance nunca chegaria à tela.
  const excluded = new Set([
    ...script,
    ...(node.mistakes ?? []).flatMap((m) => m.moves),
    ...(node.authorAlternatives ?? []).flatMap((a) => a.moves),
  ]);
 ...
 *
 * **Toda variante do defensor entra na chave.** A comparação é o que faz o gate
 * acusar ramo gerado desatualizado; um campo de fora dela é um campo que pode
 * divergir do arquivo em silêncio — e `replies` (B9/E1) é justamente o campo
 * que muda para onde a linha vai. `respostasDe` normaliza as duas escritas, o
 * que também mantém a chave estável se um expect trocar `reply` por `replies`
 * com a mesma resposta.
 */
function expectKey(expect: Expect): string {
  return JSON.stringify([expect.moves, respostasDe(expect), expect.feedback]);
}
