...
5. **`stages.guided` continua sendo saída.** A tela não tem lápis em nada da etapa 3 — o lápis ali leva ao diagrama do roteiro de onde a dica sai; a prévia deriva no navegador (`derivarTreino` + ação `lancesDoCache` lendo `content/tablebase-cache/` offline); o arquivo de verdade sai do `--write`.
...
- **Posição**: escolher existente **ou** montar (`montagem`, `fenProblem` a cada peça solta, `pieceCount ≤ 7` avisado, quem joga, `expectedResult`) + formulário dos 9 campos de proveniência (obra de `sources.json`; para posição do professor, o slug `posicoes-do-preparatorio` já existe). Nasce `candidate`; o professor marca `approved` no editor (`qaApplied` recebe "aprovada pelo professor no editor em <data>"). **Mudar a FEN de posição publicada força "salvar como nova"** — a proveniência antiga descreve outro diagrama.
...
- Editar FEN de posição publicada no lugar (proveniência viraria mentira).
- `TRILHA` como JSON editável (extras por `nivel` dão a liberdade sem entregar a lista ao formulário).
- `replies` (variantes do defensor) — continua edição à mão do JSON.
