...
**Emenda de 16/09/2026 (decisões do Doug; especificação §13.3, §18.1 e §21; plano em `docs/CURSO-DE-ABERTURA-PLANO.md`).** Quando uma abertura tem curso de abertura, o estudo do Lichess passa a ser a fonte autoral dela, e o `.pgn` do repertório é **gerado** a partir do estudo (uma partida por capítulo "Move Trainer", preâmbulo "GERADO — não editar", `[Categoria]`, `[Ordem]`, `[Fonte]`). O `.pgn` gerado continua sendo o que o compilador lê e o JSON continua derivado: não nasce uma segunda fonte JSON. Aplicar o gerado passa pelo mesmo caminho de candidato, impacto e promoção coerente deste parágrafo. A régua do repertório perde o tamanho para os 11 arquivos (sem profundidade mínima, sem teto, sem roque ou peças fora, sem `[%plano]` obrigatório) e mantém: termina em lance nosso, sem linha repetida (comentário é opcional em todo lance do move trainer desde 17/9/2026 (decisão do Doug; ver `AGENTS.md`)). As aulas do curso usam o namespace `AB-<COR>-<ABERTURA>-<BLOCO>`, com `metadados.abertura`, sem classe de finais e fora de `/finais`; a etapa nova `treinador` reusa o treino de `/aberturas` e grava em `repertorio_progresso`; a conclusão e o retomar da aula vão para uma tabela nova, aditiva (`aula_rodada`, migração 0015).
